...
public class HourlyReward {
    ...
    public void init() {
        ...
        plugin.getScheduler().runTimerAsync(() -> getTimeBiggerThan(getTimePerPayout()).forEach(uuid -> {
            ...
            if (player != null) {
                ...
                if (economy != null) {
                    player.sendMessage(
                            MessageBuilder.translatable(LangKey.HOURLY_REWARD)
                                    .placeholder("amount", economy.format(salary))
                                    .toLegacy()
                    );
                    economy.depositPlayer(player, salary);
                } else {
                    player.sendMessage(
                            MessageBuilder.translatable(LangKey.HOURLY_PENDING).toLegacy()
                    );
